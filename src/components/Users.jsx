    ...
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      ...
      {modal && (
        <Overlay onClose={()=>setModal(null)}>
          <UserModal mode={modal.mode} data={modal.data} onSave={handleSave} onClose={()=>setModal(null)} loading={loading} />
        </Overlay>
      )}
...
// ── User Form Modal ───────────────────────────────────────────────────────────
function UserModal({ mode, data, onSave, onClose, loading }) {
  const { C } = useTheme();
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:16 }}>
          ...
          <FG label="Role">
            <select style={inp} value={f.role} onChange={set('role')}>
              {getAllRoles(state.appSettings).map(r=><option key={r.key} value={r.key}>{r.label}{!r.builtin ? ' (custom)' : ''}</option>)}
            </select>
          </FG>
          {!isEdit && (
