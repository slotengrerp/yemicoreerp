...
export default function SlotStaff() {
  ...
  function voidPayrollRun() {
    ...
    dispatch({ type:'UPDATE_MODULE', mod:'payrollRuns', data:next });
    saveDBLocal({ ...db, payrollRuns:next }, state.activity);
    logActivity(dispatch, `Payroll run VOIDED: Company Staff — ${period} (${existingRun.lines?.length||0} staff, ${formatCurrency(existingRun.totalGross)} gross)${wasPaid ? ' — accrual AND payment reversing entries post automatically to the GL' : ' — reversing entry posts automatically to the GL'}`, currentUser);
