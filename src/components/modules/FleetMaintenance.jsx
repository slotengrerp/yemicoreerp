...
function printBreakdownReport(b) {
  openPrintWindow(`<!DOCTYPE html><html><head><title>Breakdown Report</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',Arial,sans-serif;font-size:12px;padding:24px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin:16px 0}.field{margin-bottom:10px}.lbl{font-size:9px;font-weight:700;text-transform:uppercase;color:#182A1C;letter-spacing:.5px;margin-bottom:2px}.val{font-size:12px;font-weight:600;color:#182A1C;border-bottom:1px solid #DDE9DE;padding-bottom:4px}.sig{display:grid;grid-template-columns:1fr 1fr 1fr;gap:40px;margin-top:40px}.sig-line{border-top:1px solid #ccc;padding-top:6px;font-size:10px;font-weight:600;color:#182A1C}@media print{body{padding:12px}}</style></head><body>${printHeader('BREAKDOWN REPORT · '+(b.reportNo||''), formatDate(b.date))}<div class="grid"><div><div class="field"><div class="lbl">Vehicle</div><div class="val">${b.regNo||'—'}</div></div><div class="field"><div class="lbl">Driver</div><div class="val">${b.driver||'—'}</div></div><div class="field"><div class="lbl">Location of Breakdown</div><div class="val">${b.location||'—'}</div></div></div><div><div class="field"><div class="lbl">Date / Time</div><div class="val">${formatDate(b.date)} ${b.time||''}</div></div><div class="field"><div class="lbl">Estimated Repair Cost</div><div class="val">₦ ${Number(b.repairCost||0).toLocaleString('en-NG')}</div></div><div class="field"><div class="lbl">Status</div><div class="val">${b.status||'—'}</div></div></div></div><div class="field"><div class="lbl">Fault Description</div><div class="val" style="white-space:pre-wrap;min-height:60px">${b.faultDescription||'—'}</div></div><div class="field" style="margin-top:12px"><div class="lbl">Action Taken</div><div class="val" style="white-space:pre-wrap;min-height:48px">${b.actionTaken||'—'}</div></div><div class="sig"><div><div class="sig-line">Reported By / Date</div></div><div><div class="sig-line">Fleet Manager / Date</div></div><div><div class="sig-line">Approved By / Date</div></div></div>${printBootstrap({landscape:true})}</body></html>`);
}
