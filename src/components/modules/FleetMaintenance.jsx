...
function printFleetRegister(fleet) {
  ...
  openPrintWindow(`<!DOCTYPE html><html><head><title>Fleet Register</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',Arial,sans-serif;font-size:12px;padding:24px}table{width:100%;border-collapse:collapse;margin-top:12px}th{background:#1A5C2A;color:#fff;padding:8px 10px;text-align:left;font-size:10px;text-transform:uppercase}td{padding:7px 10px;border-bottom:1px solid #EAF0EB;font-size:11px}@media print{body{padding:12px}}</style></head><body>${printHeader('FLEET VEHICLE REGISTER','Total Vehicles: '+fleet.length)}<table><thead><tr><th>Reg No.</th><th>Make / Model</th><th>Year</th><th>Type</th><th>Assigned To</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table>${printBootstrap({landscape:true})}</body></html>`);
}

function printServiceRecord(s) {
  openPrintWindow(`<!DOCTYPE html><html><head><title>Service Record</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',Arial,sans-serif;font-size:12px;padding:24px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin:16px 0}.field{margin-bottom:10px}.lbl{font-size:9px;font-weight:700;text-transform:uppercase;color:#182A1C;letter-spacing:.5px;margin-bottom:2px}.val{font-size:12px;font-weight:600;color:#182A1C;border-bottom:1px solid #DDE9DE;padding-bottom:4px}.sig{display:grid;grid-template-columns:1fr 1fr 1fr;gap:40px;margin-top:40px}.sig-line{border-top:1px solid #ccc;padding-top:6px;font-size:10px;font-weight:600;color:#182A1C}@media print{body{padding:12px}}</style></head><body>${printHeader('SERVICE RECORD · '+(s.serviceNo||''), formatDate(s.date))}<div class="grid"><div><div class="field"><div class="lbl">Vehicle</div><div class="val">${s.regNo||'—'}</div></div><div class="field"><div class="lbl">Mileage (km)</div><div class="val">${s.mileage||'—'}</div></div><div class="field"><div class="lbl">Service Type</div><div class="val">${s.serviceType||'—'}</div></div></div><div><div class="field"><div class="lbl">Service Provider</div><div class="val">${s.provider||'—'}</div></div><div class="field"><div class="lbl">Cost (₦)</div><div class="val">${Number(s.cost||0).toLocaleString('en-NG',{minimumFractionDigits:2,maximumFractionDigits:2})}</div></div><div class="field"><div class="lbl">Next Service (km)</div><div class="val">${s.nextServiceKm||'—'}</div></div></div></div><div class="field"><div class="lbl">Work Done / Remarks</div><div class="val" style="white-space:pre-wrap;min-height:60px">${s.workDone||'—'}</div></div><div class="sig"><div><div class="sig-line">Fleet Manager / Date</div></div><div><div class="sig-line">Driver / Date</div></div><div><div class="sig-line">Approved By / Date</div></div></div>${printBootstrap({landscape:true})}</body></html>`);
}

function printBreakdownReport(b) {
  openPrintWindow(`<!DOCTYPE html><html><head><title>Breakdown Report</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',Arial,sans-serif;font-size:12px;padding:24px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin:16px 0}.field{margin-bottom:10px}.lbl{font-size:9px;font-weight:700;text-transform:uppercase;color:#182A1C;letter-spacing:.5px;margin-bottom:2px}.val{font-size:12px;font-weight:600;color:#182A1C;border-bottom:1px solid #DDE9DE;padding-bottom:4px}.sig{display:grid;grid-template-columns:1fr 1fr 1fr;gap:40px;margin-top:40px}.sig-line{border-top:1px solid #ccc;padding-top:6px;font-size:10px;font-weight:600;color:#182A1C}@media print{body{padding:12px}}</style></head><body>${printHeader('BREAKDOWN REPORT · '+(b.reportNo||''), formatDate(b.date))}<div class="grid"><div><div class="field"><div class="lbl">Vehicle</div><div class="val">${b.regNo||'—'}</div></div><div class="field"><div class="lbl">Driver</div><div class="val">${b.driver||'—'}</div></div><div class="field"><div class="lbl">Location of Breakdown</div><div class="val">${b.location||'—'}</div></div></div><div><div class="field"><div class="lbl">Date / Time</div><div class="val">${formatDate(b.date)} ${b.time||''}</div></div><div class="field"><div class="lbl">Estimated Repair Cost</div><div class="val">₦ ${Number(b.repairCost||0).toLocaleString('en-NG',{minimumFractionDigits:2,maximumFractionDigits:2})}</div></div><div class="field"><div class="lbl">Status</div><div class="val">${b.status||'—'}</div></div></div></div><div class="field"><div class="lbl">Fault Description</div><div class="val" style="white-space:pre-wrap;min-height:60px">${b.faultDescription||'—'}</div></div><div class="field" style="margin-top:12px"><div class="lbl">Action Taken</div><div class="val" style="white-space:pre-wrap;min-height:48px">${b.actionTaken||'—'}</div></div><div class="sig"><div><div class="sig-line">Reported By / Date</div></div><div><div class="sig-line">Fleet Manager / Date</div></div><div><div class="sig-line">Approved By / Date</div></div></div>${printBootstrap({landscape:true})}</body></html>`);
}

function printHandover(h) {
  openPrintWindow(`<!DOCTYPE html><html><head><title>Vehicle Handover</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',Arial,sans-serif;font-size:12px;padding:24px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin:16px 0}.field{margin-bottom:10px}.lbl{font-size:9px;font-weight:700;text-transform:uppercase;color:#182A1C;letter-spacing:.5px;margin-bottom:2px}.val{font-size:12px;font-weight:600;color:#182A1C;border-bottom:1px solid #DDE9DE;padding-bottom:4px}.sig{display:grid;grid-template-columns:1fr 1fr;gap:60px;margin-top:48px}.sig-line{border-top:1px solid #ccc;padding-top:6px;font-size:10px;font-weight:600;color:#182A1C}@media print{body{padding:12px}}</style></head><body>${printHeader('VEHICLE HANDOVER FORM · '+(h.handoverNo||''), formatDate(h.date))}<div class="grid"><div><div class="field"><div class="lbl">Vehicle</div><div class="val">${h.regNo||'—'}</div></div><div class="field"><div class="lbl">Handed Over From</div><div class="val">${h.fromPerson||'—'}</div></div><div class="field"><div class="lbl">Mileage at Handover (km)</div><div class="val">${h.mileage||'—'}</div></div></div><div><div class="field"><div class="lbl">Handed Over To</div><div class="val">${h.toPerson||'—'}</div></div><div class="field"><div class="lbl">Department</div><div class="val">${h.department||'—'}</div></div><div class="field"><div class="lbl">Fuel Level</div><div class="val">${h.fuelLevel||'—'}</div></div></div></div><div class="field"><div class="lbl">Vehicle Condition / Remarks</div><div class="val" style="white-space:pre-wrap;min-height:60px">${h.condition||'—'}</div></div><div class="sig"><div><div class="sig-line">Handed Over By (Signature &amp; Date)</div></div><div><div class="sig-line">Received By (Signature &amp; Date)</div></div></div>${printBootstrap({landscape:true})}</body></html>`);
}
...
// 2026-08-15: no fraction-digit options meant this fell back to the
// locale's default (0-3 decimals, inconsistent), unlike every other
