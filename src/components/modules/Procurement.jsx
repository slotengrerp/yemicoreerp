...
function printPO(po) {
  ...
  openPrintWindow(`<!DOCTYPE html><html><head><title>PO ${po.poNo||''}</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',Arial,sans-serif;font-size:12px;color:#182A1C;padding:24px}table{width:100%;border-collapse:collapse}th{background:#1A5C2A;color:#fff;padding:8px 10px;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:.4px}.info-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:16px 0}.field{margin-bottom:8px}.lbl{font-size:9px;text-transform:uppercase;color:#182A1C;letter-spacing:.5px;margin-bottom:2px}.val{font-size:12px;font-weight:600;border-bottom:1px solid #DDE9DE;padding-bottom:3px}.total-row{background:#F0F8F2;font-weight:700}.sig{display:grid;grid-template-columns:1fr 1fr 1fr;gap:40px;margin-top:40px}.sig-line{border-top:1px solid #ccc;padding-top:6px;font-size:10px;font-weight:600;color:#182A1C}@media print{body{padding:12px}}</style></head><body>${printHeader('PURCHASE ORDER · ' + (po.poNo||''), formatDate(po.date))}<div class="info-grid"><div><div class="field"><div class="lbl">Supplier</div><div class="val">${po.supplier||'—'}</div></div><div class="field"><div class="lbl">Supplier Address</div><div class="val">${po.supplierAddress||'—'}</div></div><div class="field"><div class="lbl">Payment Terms</div><div class="val">${po.paymentTerms||'—'}</div></div></div><div><div class="field"><div class="lbl">Delivery Address</div><div class="val">${po.deliveryAddress||'—'}</div></div><div class="field"><div class="lbl">Delivery Date</div><div class="val">${formatDate(po.deliveryDate)||'—'}</div></div><div class="field"><div class="lbl">Status</div><div class="val">${po.status||'—'}</div></div></div></div><table style="margin-bottom:16px"><thead><tr><th>#</th><th>Description</th><th style="text-align:center">Qty</th><th style="text-align:center">Unit</th><th style="text-align:right">Unit Price</th><th style="text-align:right">Total</th></tr></thead><tbody>${itemRows}</tbody><tfoot><tr class="total-row"><td colspan="5" style="padding:8px 10px;text-align:right">Subtotal</td><td style="padding:8px 10px;text-align:right">${fmt(po.subtotal)}</td></tr><tr class="total-row"><td colspan="5" style="padding:8px 10px;text-align:right">VAT (${po.vatRate||0}%)</td><td style="padding:8px 10px;text-align:right">${fmt(po.vatAmount)}</td></tr><tr style="background:#1A5C2A;color:#fff"><td colspan="5" style="padding:10px;text-align:right;font-weight:800;font-size:13px">TOTAL</td><td style="padding:10px;text-align:right;font-weight:800;font-size:15px">${fmt(po.total)}</td></tr></tfoot></table>${po.notes?`<div style="margin-bottom:16px;padding:10px;background:#f3faf5;border-left:3px solid #1A5C2A;border-radius:4px"><div style="font-size:10px;color:#182A1C;text-transform:uppercase;margin-bottom:4px">Notes</div><div style="font-size:12px">${po.notes}</div></div>`:''}<div class="sig"><div><div class="sig-line">Prepared By / Date</div></div><div><div class="sig-line">Authorised By / Date</div></div><div><div class="sig-line">Supplier Acknowledgement / Date</div></div></div>${printBootstrap({landscape:false})}</body></html>`);
}
