...
> **⚠ DEP-1 WAS WRONG — corrected 2026-07-27, same day.** The conclusion below ("firebase.json is dead configuration") was inferred from `deploy.yml` targeting Vercel, without checking what was actually serving traffic. It is false. **`yemicoreerp.web.app` (Firebase Hosting) is the live production site users are on today**, deployed manually, and it holds the real Supabase-backed data. Meanwhile `deploy.yml` had *never* successfully deployed to Vercel at all (missing `VERCEL_TOKEN`). So the file I called dead was the live one, and the pipeline I called live was dead — exactly backwards. `firebase.json` has been restored. The lesson for future audits in this repo: verify what is actually serving production before calling any deploy config dead. `netlify.toml` remains deleted — no evidence of a Netlify site was found, but that too is an inference, not a verified fact.

**Finding DEP-1 (Low, code smell) — SUPERSEDED, SEE CORRECTION ABOVE:** Three hosting configs exist — `vercel.json`, `firebase.json`, `netlify.toml` — but `.github/workflows/deploy.yml` only ever deploys to Vercel (`vercel build` / `vercel deploy --prod`). `firebase.json` and `netlify.toml` are dead configuration; `netlify.toml`'s own comment ("SPA redirect... so React Router works") is also factually wrong — the app doesn't use React Router (confirmed: not in `package.json`, no router import anywhere in `src/`) and never has, per `App.jsx`'s custom `useState`-based navigation. Low priority, but worth deleting both dead configs (or documenting explicitly that they're kept intentionally as a migration escape hatch) to stop a future reader from assuming Netlify/Firebase are live deploy targets.
